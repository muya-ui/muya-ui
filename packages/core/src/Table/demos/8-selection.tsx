...
export default function Ellipsis() {
  ...
  const columns: ITableColumn<Album>[] = [
    ...
    },
    {
      ...
      onFilter: (value, data) => dayjs(data.date).year() > value,
...
export const meta = {
  title: '可选择表格',
  desc:
    '1. 配置`rowSelection`即可开启表格选择功能 \n\n 2. 监听`rowSelection`的`onChange`事件获取数据的变化，支持受控/非受控模式，传入`selectedRowKeys`即为受控模式 \n\n 3. 通过`getCheckboxProps`可以设置`Checkbox`的属性。若禁用了Checkbox，全选/取消全选、计算当前全选状态时会忽略disabled Checkbox',
};
