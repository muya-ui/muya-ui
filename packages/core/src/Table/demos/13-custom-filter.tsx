...
import { Table, ITableColumn, Img, Input, InlineButton } from '@muya-ui/core';
...
export default function Ellipsis() {
  ...
  const handleSearch = useCallback((text: string, method: () => void) => {
    setDescSearchText(text);
    method();
  }, []);
  ...
  const columns: ITableColumn<Album>[] = [
    ...
    },
    {
      ...
      onFilter: (value, data) => data.title === value,
    ...
    },
    {
      title: '专辑介绍',
      key: 'description',
      filterIcon: <SearchIcon style={{ marginLeft: '4px' }} />,
      render(record) {
        return descSearchText ? (
          <Highlighter
            highlightStyle={{ backgroundColor: '#ffc069', margin: '0 4px' }}
            searchWords={[descSearchText]}
            autoEscape
      ...
      renderFilterContent({ setSelectedFilterValues, selectedFilterValues, confirm }) {
        const inputValue = selectedFilterValues[0] as string;
        return (
          <Input
            inputRef={inputRef}
            placeholder="输入关键词搜索"
            value={inputValue}
            size="s"
            onPressEnter={() => handleSearch(inputValue, confirm)}
            onChange={e => {
              setSelectedFilterValues(e.target.value ? [e.target.value] : []);
            }}
          />
        );
      },
      renderFilterActions({ selectedFilterValues, confirm, reset }) {
        const inputValue = selectedFilterValues[0] as string;
        return (
          <span>
            <InlineButton onClick={() => handleSearch('', reset)}>重置</InlineButton>
            <InlineButton onClick={() => handleSearch(inputValue, confirm)} type="primary">
              确认
            </InlineButton>
          </span>
        );
  ...
  return (
    <Table<Album>
      rowSelection={{
        ...
        getCheckboxProps(d) {
          return {
            disabled: d.title === '范特西',
          };
        },
        onSelect(selected, row) {
          console.log('onSelect', selected, row);
        },
        onSelectAll(selected, rows) {
          console.log('onSelectAll', selected, rows);
        },
...
export const meta = {
  title: '自定义筛选菜单',
  desc:
    '1. 通过`column.renderFilterContent`和`column.renderFilterActions`自定义渲染逻辑 \n\n 2. 通过`column.filterVisible`和`column.onFilterVisibleChange`控制菜单的显示状态',
};
