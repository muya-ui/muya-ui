...
export const meta = {
  title: '基础用法',
  desc:
    '1. `dataSource`和`columns`为必传项\n\n 2. 在`dataSource`子项没有指定`key`的情况下，为`Table`组件设置`rowKey`属性，渲染时会自动获取`dataSource[index][rowKey]`的值作为当前key\n\n3. 表格内容默认取`dataSource[index][column.dataIndex]`，可以通过column的`render`函数自定义表格内容',
};
