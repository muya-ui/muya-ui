...
import { ITableBodyRowProps } from '../types';
...
function BodyRow(props: ITableBodyRowProps<Album>) {
  ...
  const [, drop] = useDrop({
    ...
    hover(item: DragItem, monitor: DropTargetMonitor) {
      ...
      rowProps.moveRow && rowProps.moveRow(dragIndex, hoverIndex);
  ...
  const [{ isDragging }, drag] = useDrag({
    item: { type: ItemTypes.ROW, rowIndex: index, rowKey },
    collect: (monitor: any) => ({
  ...
  drag(drop(ref));
  return (
    <tr ref={ref} key={rowKey} style={{ opacity }}>
      {children}
    </tr>
  );
}
...
export const meta = {
  title: '拖拽排序',
  desc: '集成 react-dnd 来实现拖拽排序，例子采用`react-dnd@10.0.2`书写',
};
