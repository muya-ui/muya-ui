...
import { Table, ITableColumn, Img, Input, toast, Popconfirm } from '@muya-ui/core';
...
function EditableTitle(props: EditableTitleProps) {
  ...
  if (editable) {
    ...
    return (
      <strong style={{ cursor: 'pointer' }} onClick={handleTitleClick}>
        《{title}》
      </strong>
    );
  }
...
export default function Simple() {
    ...
    },
    {
      title: (
        <Popconfirm
          ...
          placement="top"
        >
          <span style={{ cursor: 'pointer' }}>专辑名</span>
        </Popconfirm>
      ),
      key: 'title',
      width: 150,
      render: album => (
        <EditableTitle
          title={album.title}
          onChange={newTitle => handleTitleChange(album.id, newTitle)}
        />
      ),
    },
    {
  ...
  return <Table<Album> columns={columns} dataSource={albums} rowKey="title" />;
}

export const meta = {
  title: '可编辑的表格',
  desc: '通过自定义column render函数，即可定制可编辑的单元格',
};
